...
function normalizeDictationText(raw: string, bcp47: string) {
  ...
  // normalización ligera (sin cargarte mayúsculas del usuario: convertimos internamente)
  const original = s;
  const lower = s.toLowerCase();

  if (family === "es") {
    return restoreCase(original, normalizeEs(lower));
  }
  if (family === "en") {
    return restoreCase(original, normalizeEn(lower));
  }
  if (family === "de") {
    return restoreCase(original, normalizeDe(lower));
  }
 ...
 * Como nuestra normalización toca números, normalmente da igual, pero esto evita
 * cambios raros en frases enteras.
 */
function restoreCase(original: string, normalizedLower: string) {
  // Si el original parece todo en minúsculas, devolvemos tal cual.
  const hasUpper = /[A-ZÁÉÍÓÚÜÑÄÖ]/.test(original);
  if (!hasUpper) return normalizedLower;

  // Mantén mayúscula inicial si la tenía
...
function replaceWholeWords(s: string, map: Record<string, string>) {
  // Sustitución por palabras completas, priorizando claves largas
...
function normalizeEs(s: string) {
  ...
  // 2) "primero de enero" => "1 de enero"
  out = out.replace(new RegExp(`\\bprimero\\b(?=\\s+de\\s+(${ES_MONTHS.join("|")}))`, "g"), "1");

  // 3) Números (0..59) para horas/minutos y días simples
  // Ojo: primero frases largas ("treinta y uno"), luego palabras sueltas
  ...
  out = out.replace(/\bcuarenta y cinco\b/g, "45"); // por frecuencia
  out = out.replace(/\bcuarenta y\b/g, "40 y"); // deja hueco para "cuarenta y cinco" si no lo pilló antes

  out = replaceWholeWords(out, ES_NUM_0_59);

  // 4) "a las seis y media" => "a las 6 y media" (ya lo hace el mapa)
  // 5) "a las 6 y cuarto" lo dejamos (tu parser puede interpretarlo o pedir confirmación)
...
function esWordsToNumber_0_99(phrase: string): number | null {
  ...
  // directo si está en 0..59
  if (ES_NUM_0_59[p] != null) return Number(ES_NUM_0_59[p]);

  // 60..99 (básico): "sesenta y cinco", "setenta y dos", etc.
  const m = p.match(/^(sesenta|setenta|ochenta|noventa)(?:\s+y\s+(uno|una|dos|tres|cuatro|cinco|seis|siete|ocho|nueve))?$/);
  if (m) {
    const tensWord = m[1];
    const onesWord = m[2];
    const tens =
  ...
  // "treinta", "cuarenta", etc. sin unidades
  const tensOnly =
    p === "treinta" ? 30 :
    p === "cuarenta" ? 40 :
    p === "cincuenta" ? 50 :
...
function normalizeEn(s: string) {
  ...
  // 2) numeric words (0..59) for times/days
...
function enWordsToNumber_0_99(phrase: string): number | null {
  ...
  // Normalize hyphens into spaces for lookup
  const key = p.replace(/-/g, " ");
  if (EN_NUM_0_59[key] != null) return Number(EN_NUM_0_59[key]);

  // tens + ones (60..99) basic
...
function normalizeDe(s: string) {
  ...
  // 1) years 2000-2099:
  // "zweitausend fünfundzwanzig" => 2025
  // "zwei tausend fünfundzwanzig" => 2025
  ...
  // 3) common time words (optional): "uhr" stays, "halb" etc left as-is (parser/confirm)
  return out;
}
...
function deWordsToNumber_0_99(phrase: string): number | null {
  ...
  // 60..99 patterns like "sechzig", "siebzig", "achtzig", "neunzig" (+ optional "eins"/"zwei"...)
  // Note: dictation often gives "fünfundsechzig" (one word) instead of "sechzig fünf".
  // We cover the one-word form via basic regex.
  const compact = p.replace(/\s+/g, "");

  // handle "fünfundsechzig" style: "<ones>und<ten>"
...
export function useSpeechDictation(opts: Options = {}) {
  ...
  const start = useCallback(
    (onText: (chunk: DictationChunk) => void, langOverride?: string) => {
      ...
      // El idioma debe ser BCP-47; lo seteamos justo antes de start()
      ...
      rec.onresult = (event: any) => {
        ...
        else lastInterimRef.current = interim;

        onText({ finalText, interimText: interim });
      };

      // start() puede lanzar si ya está iniciado
      try {
        rec.start();
      } catch {
        // ignore
